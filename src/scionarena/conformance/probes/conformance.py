...
class R1SharedLinkSensitivity(Probe):
    ...
    remedy = ("Predictions must respond to the state of the links a path "
    ...
    def run(self, model, world, rng):
        ...
        counts = {i: len(world.paths_using(i)) if hasattr(world, "paths_using")
        ...
        if not shared:
            return self.result(Status.NOT_APPLICABLE,
                               "No interface is used by a strict subset of paths.")
        ...
        du = mean([_rel(base[p].cost(), after[p].cost()) for p in users if p in after]) if users else 0.0
        do = mean([_rel(base[p].cost(), after[p].cost()) for p in others if p in after]) if others else 0.0

        ev = dict(perturbed_interface=iid, n_users=len(users), n_others=len(others),
                  mean_change_on_users=round(du, 4), mean_change_on_others=round(do, 4))
        if du < 0.02:
            return self.result(Status.FAIL,
                               "Degrading a shared link barely moved the predictions "
                               "of the paths that use it.", score=0.0, **ev)
        if du <= do * 1.2:
            return self.result(Status.WEAK,
                               "Paths using the degraded link moved no more than paths "
                               "that do not. The model is reacting, but not through "
                               "the graph.", score=0.35, **ev)
        return self.result(Status.PASS,
                           f"Affected paths moved {du/max(do,1e-9):.1f}x more than "
                           f"unaffected ones.", score=min(1.0, du / max(do, 1e-9) / 5), **ev)
...
class R2UnseenPathComposition(Probe):
    ...
    remedy = ("A path assembled from links you have observed should be "
    ...
    def run(self, model, world, rng):
        ...
        if newp.path_id not in preds:
            return self.result(Status.FAIL,
                               "Model returned no prediction for a path built from "
                               "links it has observed.", score=0.0)
        ...
        err = _rel(v, truth)
        ev = dict(predicted_latency_ms=round(v, 2), true_latency_ms=round(truth, 2),
                  relative_error=round(err, 3))
        if err > 1.0:
            return self.result(Status.WEAK,
                               "Produced a prediction, but off by more than 100%.",
                               score=0.3, **ev)
        return self.result(Status.PASS,
                           f"Composed a prediction within {err*100:.0f}% of truth.",
                           score=max(0.0, 1.0 - err), **ev)
...
class R3MissingnessDiscrimination(Probe):
    ...
    remedy = ("Pair every telemetry feature with an observed/not-observed mask. "
    ...
    def run(self, model, world, rng):
        ...
        for _ in range(WARMUP_STEPS):
            ...
            obs = [o if o.path_id != target.path_id else
                   type(o)(t=o.t, path_id=o.path_id, latency_ms=None,
                           throughput_mbps=o.throughput_mbps, loss=o.loss, source=o.source)
                   for o in obs]
            model.observe(obs, world.snapshot())
            world.step()
        ...
        for _ in range(WARMUP_STEPS):
            obs = world.observe(demand=world.uniform_demand())
            obs = [o if o.path_id != target.path_id else
                   type(o)(t=o.t, path_id=o.path_id, latency_ms=0.0,
                           throughput_mbps=o.throughput_mbps, loss=o.loss, source=o.source)
                   for o in obs]
            model.observe(obs, world.snapshot())
            world.step()
        ...
        ev = dict(pred_when_missing=round(a, 3) if a == a else None,
...
class R4UnseenInterfaces(Probe):
    ...
    remedy = ("Replace identity lookups with structural features computable "
    ...
    def run(self, model, world, rng):
        ...
        try:
            ...
            return self.result(Status.FAIL,
                               f"Raised on a topology containing new interfaces: "
                               f"{type(exc).__name__}: {exc}", score=0.0,
                               new_interfaces=new_ifaces)

        if newp.path_id not in after:
            return self.result(Status.FAIL,
                               "Silently dropped the path made of unseen interfaces.",
                               score=0.0, new_interfaces=new_ifaces)
        v = after[newp.path_id].latency_ms.point
        if math.isnan(v):
            return self.result(Status.FAIL, "Returned NaN for the unseen path.",
                               score=0.0, new_interfaces=new_ifaces)
        ...
        ev = dict(n_paths_before=n_before, n_paths_after=len(after),
                  new_interfaces=new_ifaces,
                  spread_unseen=round(sp_new, 3), spread_known=round(sp_old, 3))
...
class R5Distributional(Probe):
    ...
    def run(self, model, world, rng):
        ...
        ev = dict(fraction_distributional=round(frac, 3),
                  quantile_order_violations=bad_order[:5])
        ...
        if frac < 0.99:
            return self.result(Status.WEAK,
                               ...
                               score=frac, **ev)
        return self.result(Status.PASS, "All predictions are distributional and ordered.",
                           score=1.0, **ev)
...
class R6DemandConditioning(Probe):
    ...
    remedy = ("Take demand as an explicit input. Without it the model cannot "
    ...
    def run(self, model, world, rng):
        ...
        ev = dict(loaded_path=target, change_on_loaded=round(d_target, 4),
...
class R7Monotonicity(Probe):
    ...
    remedy = ("Constrain the demand pathway architecturally. Monotonicity is "
    ...
    def run(self, model, world, rng):
        ...
        drops = [(levels[i + 1], round(costs[i + 1] - costs[i], 6))
                 for i in range(len(costs) - 1) if costs[i + 1] < costs[i] - 1e-9]
        span = max(costs) - min(costs)
        ev = dict(levels=levels, costs=[round(c, 3) for c in costs],
                  violations=drops, total_span=round(span, 3))

        if span < 1e-6:
            return self.result(Status.FAIL,
                               "Cost is flat across the whole demand sweep; the model is "
                               "not conditioning on demand at all.", score=0.0, **ev)
        if drops:
            frac = len(drops) / (len(costs) - 1)
            return self.result(Status.FAIL,
                               f"Predicted cost *fell* as load rose at {len(drops)} of "
                               f"{len(costs)-1} steps.", score=max(0.0, 1 - frac), **ev)
        return self.result(Status.PASS, "Cost rose monotonically across the sweep.",
                           score=1.0, **ev)
...
class R8EmitsAssignment(Probe):
    ...
    remedy = ("Publish probabilities and let hosts sample. Every host that reads "
    ...
    def run(self, model, world, rng):
        ...
        w = adv.normalised()
        tot = sum(w.values())
        ev = dict(n_paths=len(w), max_weight=round(adv.max_weight, 4),
...
class R9SelfConsistency(Probe):
    ...
    remedy = ("Solve for a fixed point before publishing: predict, compute the "
    ...
    def run(self, model, world, rng):
        ...
        if u2:
            ...
            rank_shift = 0.0 if best_after == top else abs(w1.get(top, 0.0) - w1.get(best_after, 0.0))

        ev = dict(total_variation_between_rounds=round(tv, 4),
                  top_path=top, top_path_cost_drift_under_own_advice=round(drift, 4),
        ...
        if a1.solver_converged is False:
            return self.result(Status.WEAK,
                               ...
                               "fallback path should be exercised.", score=0.3, **ev)
        if tv > 0.15:
            return self.result(Status.FAIL,
                               f"Advisory moved by {tv:.2f} total variation between two "
                               f"consecutive asks on identical state.", score=0.0, **ev)
...
class R10StalenessResponse(Probe):
    ...
    remedy = ("Make the concentration of the advisory a decreasing function of "
    ...
    def run(self, model, world, rng):
        ...
        fresh_conf = mean([p.confidence for p in fresh_pred.values()
                           if p.confidence is not None]) if any(
                               p.confidence is not None for p in fresh_pred.values()) else None
        ...
        stale_conf = mean([p.confidence for p in stale_pred.values()
                           if p.confidence is not None]) if any(
                               p.confidence is not None for p in stale_pred.values()) else None

        ev = dict(fresh_entropy=round(fresh.normalised_entropy, 4),
        ...
        less_sure = (fresh_conf is not None and stale_conf is not None
                     and stale_conf < fresh_conf - 1e-3)

        if not widened and not less_sure:
            return self.result(Status.FAIL,
                               "After 600 s with no new measurements the advisory is just "
                               "as concentrated and just as confident.", score=0.0, **ev)
        if widened and less_sure:
            return self.result(Status.PASS,
                               "Both the advisory and the reported confidence relax as "
                               "information ages.", score=1.0, **ev)
        return self.result(Status.PASS if widened else Status.WEAK,
                           "Advisory relaxes with age." if widened
                           else "Confidence falls but the advisory stays as concentrated.",
                           score=0.7 if widened else 0.5, **ev)


ALL_PROBES = [
    R1SharedLinkSensitivity(), R2UnseenPathComposition(), R3MissingnessDiscrimination(),
    R4UnseenInterfaces(), R5Distributional(), R6DemandConditioning(),
    R7Monotonicity(), R8EmitsAssignment(), R9SelfConsistency(), R10StalenessResponse(),
]
